...
enum class RatingPartitionPolicy : uint8_t {
  ...
  block_quality,
  dangling_nodes,
  dangling_nodes_new_initial_partition
};
...
static std::ostream& operator<< (std::ostream& os, const EvoCombineStrategy& combine) {
  switch (combine) {
    ...
    case EvoCombineStrategy::block_quality: return os << "block_quality";
...
static std::ostream& operator<< (std::ostream& os, const EvoMutateStrategy& mutation) {
  switch (mutation) {
    ...
    case EvoMutateStrategy::dangling_nodes: return os << "dangling_nodes";
    case EvoMutateStrategy::dangling_nodes_new_initial_partition: return os << "dangling_nodes_new_initial_partition";    
...
static std::ostream& operator<< (std::ostream& os, const RatingPartitionPolicy& policy) {
  switch (policy) {
    ...
    case RatingPartitionPolicy::block_quality: return os << "block_quality";
    case RatingPartitionPolicy::dangling_nodes: return os << "dangling_nodes";
    case RatingPartitionPolicy::dangling_nodes_new_initial_partition: return os << "dangling_nodes_new_initial_partition";
      // omit default case to trigger compiler warning for missing cases
  }
...
static EvoMutateStrategy mutateStrategyFromString(const std::string& strat) {
  ...
  } else if (strat == "dangling_nodes") {
    return EvoMutateStrategy::dangling_nodes;
  } else if (strat == "dangling_nodes_new_initial_partition") {
    return EvoMutateStrategy::dangling_nodes_new_initial_partition;
  }
...
static EvoCombineStrategy combineStrategyFromString(const std::string& strat) {
  ...
  } else if (strat == "block_quality") {
    return EvoCombineStrategy::block_quality;
  }
  LOG << "No valid combine strategy. " << strat;
  exit(0);
}
...
static RatingPartitionPolicy ratingPartitionPolicyFromString(const std::string& partition) {
  ...
  } else if (partition == "block_quality") {
    return RatingPartitionPolicy::block_quality;
  } else if (partition == "dangling_nodes") {
    return RatingPartitionPolicy::dangling_nodes;
  } else if (partition == "dangling_nodes_new_initial_partition") {
    return RatingPartitionPolicy::dangling_nodes_new_initial_partition;
  }
