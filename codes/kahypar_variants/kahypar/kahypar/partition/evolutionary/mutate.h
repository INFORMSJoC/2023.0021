...
#include "kahypar/partition/evolutionary/dangling_nodes.h"
...
Individual danglingNodes(Hypergraph& hg, const Individual& in,
  ...
  temporary_context.evolutionary.action =
    Action(meta::Int2Type<static_cast<int>(EvoDecision::mutation)>(),
           meta::Int2Type<static_cast<int>(EvoMutateStrategy::dangling_nodes)>());

  temporary_context.coarsening.rating.rating_function = RatingFunction::heavy_edge;
  temporary_context.coarsening.rating.partition_policy = RatingPartitionPolicy::dangling_nodes;

  temporary_context.evolutionary.dangling_nodes = computeDanglingNodes(hg, in);
...
Individual danglingNodesWithNewInitialPartitioning(Hypergraph& hg, const Individual& in,
                                            const Context& context) {
  const HighResClockTimepoint start = std::chrono::high_resolution_clock::now();
  ...
  temporary_context.evolutionary.action =
    Action(meta::Int2Type<static_cast<int>(EvoDecision::mutation)>(),
           meta::Int2Type<static_cast<int>(EvoMutateStrategy::dangling_nodes_new_initial_partition)>());

  temporary_context.coarsening.rating.rating_function = RatingFunction::heavy_edge;
  temporary_context.coarsening.rating.partition_policy = RatingPartitionPolicy::dangling_nodes_new_initial_partition;
