...
Individual blockQuality(Hypergraph& hg, const Context& context, const Population& population) {
  const HighResClockTimepoint start = std::chrono::high_resolution_clock::now();
  hg.reset();
  Context temporary_context(context);
  ...
  temporary_context.evolutionary.action =
    Action { meta::Int2Type<static_cast<int>(EvoDecision::combine)>(),
             meta::Int2Type<static_cast<int>(EvoCombineStrategy::block_quality)>() };
  temporary_context.coarsening.rating.rating_function = RatingFunction::heavy_edge;
  temporary_context.coarsening.rating.partition_policy = RatingPartitionPolicy::block_quality;
  temporary_context.coarsening.contraction_limit_multiplier = context.evolutionary.block_quality_contraction_limit_multiplier;

  const auto& parents = population.tournamentSelect();
  //temporary_context.evolutionary.block_quality = computeBlockQuality(hg, parents);
  Individuals individuals;
  individuals.push_back(parents.first);
  individuals.push_back(parents.second);
  temporary_context.evolutionary.block_quality = computeBlockQuality(hg, individuals);
  //temporary_context.evolutionary.block_quality = computeBlockQuality(hg, population.randomIndividuals((size_t)std::max(3.0, std::sqrt(population.size()))));
  //temporary_context.evolutionary.block_quality = computeBlockQuality(hg, population.listOfBest((size_t)std::max(3.0, std::sqrt(population.size()))));
  ...
  Individual child = Individual(hg, context);
  return child;
}
