...
class Population {
 ...
 public:
  ...
  inline Individuals randomIndividuals(const size_t& amount) const {
    ...
    Randomize::instance().shuffleVector(random_numbers, _individuals.size());

    Individuals individuals;
    for (size_t i = 0; i < amount; ++i) {
      individuals.push_back(_individuals[random_numbers[i]]);
    }
    return individuals;
  }
