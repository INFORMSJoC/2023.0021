 ...
 private:
  ...
  inline void performCombine(Hypergraph& hg, const Context& context) {
    ...
    switch (context.evolutionary.combine_strategy) {
      ...
      case EvoCombineStrategy::block_quality: {
          Individual ind = combine::blockQuality(hg, context, _population);
          if (metrics::imbalance(hg, context) <= context.partition.epsilon) {
           size_t insert_position = _population.insert(std::move(ind), context);
          	verbose(context, insert_position);
          }
          break;
        }
  ...
  inline void performMutation(Hypergraph& hg, const Context& context) {
    ...
    switch (context.evolutionary.mutate_strategy) {
      ...
      case EvoMutateStrategy::vcycle:
        ...
        verbose(context, mutation_position);
        break;
      case EvoMutateStrategy::dangling_nodes:
        _population.insert(
          mutate::danglingNodes(hg, _population.individualAt(mutation_position), context), context);
        verbose(context, mutation_position);
        break;
      case EvoMutateStrategy::dangling_nodes_new_initial_partition:
        _population.insert(
          mutate::danglingNodesWithNewInitialPartitioning(hg, _population.individualAt(mutation_position), context), context);
        verbose(context, mutation_position);
        break;
