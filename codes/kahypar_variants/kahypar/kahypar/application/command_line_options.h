...
po::options_description createEvolutionaryOptionsDescription(Context& context,
  ...
  evolutionary_options.add_options()
    ...
    "default: 0.5)")
    ("block-quality-chance",
    po::value<float>()->value_name("<float>")->notifier(
      [&](const float& block_chance) {
      context.evolutionary.block_quality_chance = block_chance;
    }),    
    "The Chance of a block quality selected as operation\n"
    "default: 0.33)")
    ("block-quality-contraction-limit",
    po::value<float>()->value_name("<float>")->notifier(
      [&](const float& block_quality_c_l) {
      context.evolutionary.block_quality_contraction_limit_multiplier = block_quality_c_l;
    }),    
    "The contraction limit for the block quality operator\n"
    "default: 1)")
