...
config_path = os.path.abspath("./kahypar/config/km1_kKaHyPar_sea20.ini")
...
for sub_folder in tqdm(sub_folders):
    ...
    result_folder = "{}/{}/results/kahypar".format(main_folder, sub_folder)  
    
    if not os.path.exists(result_folder):
        os.mkdir(result_folder)
    
    os.chdir(instance_folder)
    for filename in tqdm(sorted(os.listdir())):
        ...
        command = """{} -h {}  -k {} -e 0 -o km1  -m direct -p {} --use-individual-part-weights 1 --part-weights {} --i-bp-early-restart 0 --i-bp-late-restart 0""".format(binary_path, instance_folder +  filename, k, config_path, tw)
        ...
        for _ in range(1):
            ...
            for i in range(2,k+1):
                if sum(target_weights[:i]) >= total_weight:
                    diff = sum(target_weights[:i]) - total_weight
                    tw = " ".join([str(x) for x in target_weights[:i]])
                    command = """{} -h {}  -k {} -e 0 -o km1  -m direct -p {} --use-individual-part-weights 1 --part-weights {} --i-bp-early-restart 0 --i-bp-late-restart 0""".format(binary_path, instance_folder +  filename, i, config_path, tw)
                    for xxxxx in range(diff + 1):
        ...
        ets[res_filename].append(total_et)
